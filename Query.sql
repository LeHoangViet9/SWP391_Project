-- =============================================================================
-- 1. DỌN SẠCH DỮ LIỆU & RESET ID TỰ TĂNG (ĐÃ BỎ BẢNG equipment_checks)
-- =============================================================================
TRUNCATE TABLE
    room_state_history, room_img, equipment_images,
    repair_requests, equipments, invoices, bookings, customers,
    room, room_type, user_permissions, role_permissions, permission, users, roles
    RESTART IDENTITY CASCADE;

-- =============================================================================
-- 2. CẬP NHẬT RÀNG BUỘC (CONSTRAINTS) ĐỒNG BỘ VỚI JAVA ENUM
-- =============================================================================
...
ALTER TABLE room ADD CONSTRAINT room_room_status_check CHECK (room_status IN ('AVAILABLE', 'MAINTENANCE', 'INACTIVE', 'RESERVED', 'CLEANING', 'DIRTY', 'OCCUPIED', 'READY', 'OUT_OF_ORDER'));
...
-- =============================================================================
-- CẬP NHẬT RÀNG BUỘC CHO BẢNG ROOM (Bổ sung CHECKOUT_PENDING)
-- =============================================================================
ALTER TABLE room DROP CONSTRAINT IF EXISTS room_room_status_check;
...
-- =============================================================================
-- CẬP NHẬT RÀNG BUỘC CHO BẢNG ROOM_STATE_HISTORY
-- =============================================================================
-- Hủy bỏ ràng buộc cũ
ALTER TABLE room_state_history DROP CONSTRAINT IF EXISTS room_state_history_triggered_by_process_check;

-- Tạo lại ràng buộc mới, chấp nhận cả giá trị từ script mẫu cũ và Enum Java mới
ALTER TABLE room_state_history ADD CONSTRAINT room_state_history_triggered_by_process_check
    CHECK (triggered_by_process IN (
                                    'HOUSEKEEPING', 'MAINTENANCE', 'CHECKIN', 'CHECKOUT', 'SYSTEM', -- Giá trị phục vụ test script
                                    'TASK_CLEANING', 'TASK_IN_PROGRESS', 'TASK_COMPLETION', 'TASK_CANCELLATION', 'TASK_SKIPPED', 'TASK_MAINTENANCE' -- Enum Java
        ));
-- 1. Đồng bộ cột triggered_by_process với Enum ProcessTrigger trong Java
ALTER TABLE room_state_history DROP CONSTRAINT IF EXISTS room_state_history_triggered_by_process_check;
ALTER TABLE room_state_history ADD CONSTRAINT room_state_history_triggered_by_process_check
    CHECK (triggered_by_process IN (
...
-- 2. Đồng bộ cột previous_state với Enum RoomStatus mới
...
-- 3. Đồng bộ cột current_state với Enum RoomStatus mới
...
-- =============================================================================
-- 3. CHÈN DỮ LIỆU DANH MỤC CƠ BẢN (ROLES & PERMISSIONS)
-- =============================================================================
...
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r, permission p
WHERE r.role_name = 'RECEPTIONIST' AND p.name IN ('ROOM_VIEW', 'ROOM_TYPE_VIEW', 'CUSTOMER_VIEW', 'CUSTOMER_CREATE', 'CUSTOMER_UPDATE', 'BOOKING_VIEW', 'BOOKING_CREATE', 'BOOKING_UPDATE', 'INVOICE_VIEW', 'INVOICE_CREATE', 'INVOICE_UPDATE', 'FEEDBACK_VIEW', 'EQUIPMENT_VIEW');

INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r, permission p
WHERE r.role_name = 'HOUSEKEEPER' AND p.name IN ('ROOM_VIEW', 'ROOM_UPDATE', 'HOUSEKEEPING_VIEW', 'HOUSEKEEPING_CREATE', 'HOUSEKEEPING_UPDATE', 'EQUIPMENT_VIEW', 'MAINTENANCE_VIEW', 'MAINTENANCE_CREATE');

INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r, permission p
WHERE r.role_name = 'MAINTENANCE' AND p.name IN ('ROOM_VIEW', 'EQUIPMENT_VIEW', 'EQUIPMENT_CREATE', 'EQUIPMENT_UPDATE', 'MAINTENANCE_VIEW', 'MAINTENANCE_CREATE', 'MAINTENANCE_UPDATE', 'MAINTENANCE_DELETE');

INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r, permission p
WHERE r.role_name = 'CUSTOMER' AND p.name IN ('BOOKING_VIEW_OWN', 'ROOM_TYPE_VIEW', 'FEEDBACK_CREATE', 'INVOICE_VIEW');

-- =============================================================================
-- 4. CHÈN DỮ LIỆU TÀI KHOẢN & KHÁCH HÀNG (USERS & CUSTOMERS)
-- =============================================================================
...
-- =============================================================================
-- 5. CHÈN DỮ LIỆU PHÒNG & LOẠI PHÒNG (ROOM_TYPE & ROOM)
-- =============================================================================
...
INSERT INTO invoices (booking_id, amount, payment_status, payment_method, paid_at) VALUES
                                                                                       (1, 1000000.00, 'PENDING', NULL, NULL), (2, 1400000.00, 'PENDING', NULL, NULL),
                                                                                       (3, 2000000.00, 'PAID', 'CASH', NOW()), (4, 2400000.00, 'PAID', 'TRANSFER', NOW()),
                                                                                       (5, 3000000.00, 'PAID', 'CARD', NOW()), (6, 1600000.00, 'PAID', 'CASH', NOW()),
                                                                                       (7, 6000000.00, 'PAID', 'TRANSFER', NOW()), (8, 24000000.00, 'PAID', 'CARD', NOW()),
                                                                                       (9, 3000000.00, 'PENDING', NULL, NULL), (10, 4400000.00, 'PENDING', NULL, NULL),
                                                                                       (11, 1100000.00, 'PAID', 'CASH', NOW()), (12, 4000000.00, 'PAID', 'TRANSFER', NOW()),
                                                                                       (13, 48000000.00, 'PENDING', NULL, NULL), (14, 800000.00, 'PENDING', NULL, NULL),
                                                                                       (15, 1800000.00, 'CANCELLED', NULL, NULL);

-- =============================================================================
-- 7. CHÈN DỮ LIỆU THIẾT BỊ & BẢO TRÌ (EQUIPMENTS & REPAIRS)
-- =============================================================================

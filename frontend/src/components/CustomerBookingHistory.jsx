...
export default function CustomerBookingHistory() {
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      ...
      notify(err.message || ('Failed to save feedback'), 'error');
    } finally {
  ...
  const handleCancelBooking = (booking) => {
    ...
    setCancelReason('Trip plan changed');
    setCancelError('');
  };
  ...
  const confirmCancelBooking = async () => {
    ...
    try {
      ...
      notify(false ? `Cancelled successful đơn đặt phòng #${cancelModalTarget.id}.` : `Successfully cancelled booking #${cancelModalTarget.id}.`);
          ...
          <span className={`inline-flex whitespace-nowrap rounded-full px-2.5 py-1 text-xs font-semibold ${STATUS_COLORS[status] || 'bg-stone-100 text-stone-600'}`}>
            {isPendingPayment && remainingSeconds !== null ? (
              <span className="inline-flex items-center gap-1.5 tabular-nums">
                ...
                {isPaymentExpired
                  ? ('Payment expired')
                  : `${'Pay within'} ${formatCountdown(remainingSeconds)}`}
              </span>
          ...
          <div className="flex flex-col gap-2">
            ...
            {hasFeedback && (
              <button
                ...
                title={"View review details or response"}
              >
  ...
  return (
    <div className="space-y-4">
      ...
      {/* thanh Search & Filter Status & Ngày Tháng */}
      <div className="flex flex-col gap-3 rounded-xl border border-stone-200 bg-white p-3.5 shadow-xs lg:flex-row lg:items-center lg:justify-between">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            placeholder={'Search by Booking #ID or Room Type name...'}
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 border border-stone-200 rounded-lg px-2.5 py-1.5 bg-stone-50 text-xs">
            ...
            />
            <span className="text-[11px] font-semibold text-slate-500 ml-1">{'To:'}</span>
            <input
              type="date"
              value={endDate}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl p-6 space-y-4">
            <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
              ...
              {editFeedbackTarget
                ? (false ? `Chỉnh sửa đánh giá #${editFeedbackTarget.id}` : `Edit Review #${editFeedbackTarget.id}`)
                : (false ? `Review phòng nghỉ #${reviewTarget.id}` : `Review Booking #${reviewTarget.id}`)}
            </h3>

            <p className="text-xs text-slate-500">
              {false ? `Room Type: ${reviewTarget.roomTypeName}` : `Room Type: ${reviewTarget.roomTypeName}`}
            </p>
            ...
            <form onSubmit={handleSubmitReview} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={'Write your review here (max 255 chars)...'}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                >
                  {submittingReview 
                    ? ('Saving...') 
                    : (editFeedbackTarget ? ('Update') : ('Submit'))}
                </button>
              </div>
            </form>
          </div>
                ...
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <span className="px-2 py-0.5 rounded text-[10px] font-extrabold uppercase tracking-wide bg-[#bfa15f]/10 text-[#bfa15f] border border-[#bfa15f]/20">
                      {false ? `Mục: ${t(`feedback.category.${viewReplyTarget.category}`) || viewReplyTarget.category}` : `Category: ${viewReplyTarget.category}`}
                    </span>
              ...
              {viewReplyTarget.reply ? (
                ...
                <div className="bg-stone-50/50 p-4 rounded-xl border border-stone-200/60 border-dashed text-center">
                  <p className="text-xs text-slate-400 italic">
                    {'Review is waiting for management approval and response.'}
                  </p>
                </div>
              )}
            </div>
      ...
      {/* Modal Cancel Bookings của Customers */}
      <Modal
        open={!!cancelModalTarget}
        title={false ? `Confirm hủy đơn đặt phòng #${cancelModalTarget?.id}` : `Confirm Cancel Booking #${cancelModalTarget?.id}`}
        ...
        <div className="space-y-4">
          <div className="flex items-start gap-3 p-3.5 bg-red-50 border border-red-200 rounded-xl">
            ...
            <div>
              ...
              <p className="text-xs text-red-700 mt-0.5 leading-relaxed">
                {false
                  ? 'Khi hoàn tất hủy đơn, các phòng đã chọn sẽ tự động được trả lại hệ thống và khách hàng khác có thể đặt ngay lập tức. Actions này không thể hoàn tác.'
          ...
          {cancelModalTarget && (
            <div className="p-4 bg-stone-50 border border-stone-200 rounded-xl space-y-2 text-xs">
              ...
              <div className="flex justify-between items-center text-slate-700 font-medium">
                <span className="text-slate-500">{'Stay Period:'}</span>
          ...
          <div>
            ...
            >
              <option value="Thay đổi kế hoạch du lịch">{'Change of travel plans'}</option>
              <option value="Tìm được lựa chọn chỗ ở khác">{'Found alternative accommodation'}</option>
              <option value="Reason cá nhân đột xuất">{'Personal emergency'}</option>
              <option value="Khác">{'Other reason'}</option>
            </select>
          </div>
      ...
      <Modal isOpen={!!deleteFeedbackId} onClose={() => setDeleteFeedbackId(null)}>
        <div className="p-6 space-y-4 max-w-md">
          ...
          <p className="text-sm text-slate-600 leading-relaxed">
            {false 
              ? 'Bạn có chắc chắn muốn xóa bài đánh giá chất lượng dịch vụ này không? Review của bạn sẽ được ẩn (xóa mềm) khỏi giao diện hiển thị.' 

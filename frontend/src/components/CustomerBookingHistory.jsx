...
export default function CustomerBookingHistory() {
  ...
  const confirmCancelBooking = async () => {
    ...
    try {
      ...
      notify(`Successfully cancelled booking #${cancelModalTarget.id}.`);
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl p-6 space-y-4">
            <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
              ...
              {editFeedbackTarget
                ? (`Edit Review #${editFeedbackTarget.id}`)
                : (`Review Booking #${reviewTarget.id}`)}
            </h3>

            <p className="text-xs text-slate-500">
              {`Room Type: ${reviewTarget.roomTypeName}`}
            </p>
      ...
      <Modal
        ...
        title={`Confirm Cancel Booking #${cancelModalTarget?.id}`}

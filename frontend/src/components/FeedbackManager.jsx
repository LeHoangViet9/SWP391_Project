...
export default function FeedbackManager() {
  ...
  const fetchFeedbacks = useCallback(async () => {
    ...
    try {
      ...
      notify(err.message || ('Failed to load feedbacks'), 'error');
    } finally {
      setLoading(false);
    }
  }, [search, filterRating, filterStatus, filterCategory, page, locale, false]);
  ...
  const handleSaveReply = async (e) => {
    ...
    try {
      ...
      notify('Feedback replied successfully!');
      setReplyTarget(null);
      setReplyText('');
      fetchFeedbacks();
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-white/50 mt-1">
            {'View and reply to service quality ratings from staying guests.'}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row justify-between gap-4 bg-[#112240] p-4 rounded-2xl border border-white/[0.08]">
        <div className="flex flex-col sm:flex-row gap-3 flex-1">
          ...
          >
            <option value="" className="bg-[#112240] text-white">{'All Categories'}</option>
            <option value="Room" className="bg-[#112240] text-white">{'Room'}</option>
            <option value="Service" className="bg-[#112240] text-white">{'Service'}</option>
            <option value="Cleanliness" className="bg-[#112240] text-white">{'Cleanliness'}</option>
            <option value="Staff" className="bg-[#112240] text-white">{'Staff'}</option>
          </select>
          ...
          <select
            ...
            className="px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-xs font-semibold text-white focus:border-[#bfa15f] outline-none transition-all cursor-pointer"
          >
            <option value="" className="bg-[#112240] text-white">{'All statuses'}</option>
            <option value="PENDING" className="bg-[#112240] text-white">{'Pending'}</option>
            <option value="REVIEWED" className="bg-[#112240] text-white">{'Reviewed'}</option>
          </select>
        </div>
      ...
      <div className="space-y-4">
        {loading ? (
          <div className="bg-[#112240] border border-white/[0.08] p-12 text-center rounded-2xl">
            <span className="text-sm text-white/40">{'Loading feedbacks...'}</span>
          </div>
        ) : feedbacks.length === 0 ? (
          <div className="bg-[#112240] border border-white/[0.08] p-12 text-center rounded-2xl">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between bg-[#112240] p-4 rounded-2xl border border-white/[0.08] mt-4">
          <span className="text-xs text-white/40">
            {false ? `Page ${page + 1} / ${totalPages}` : `Page ${page + 1} of ${totalPages}`}
          </span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-[#112240] border border-white/10 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl p-6 space-y-4">
            <h3 className="text-base font-bold text-white flex items-center gap-2">
              ...
              {false ? `Response khách hàng ${replyTarget.customerName}` : `Reply to ${replyTarget.customerName}`}
            </h3>
          ...
          <div className="relative bg-[#112240] border border-red-500/30 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl p-6 space-y-4 animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="bg-white/5 border border-white/5 rounded-xl p-3.5 space-y-1">
              <p className="text-xs font-semibold text-white/80">
                {false ? `Review từ: ${deleteTarget.customerName}` : `Review by: ${deleteTarget.customerName}`}
              </p>
            ...
            <p className="text-xs text-white/70 leading-relaxed">
              {false
                ? 'Review này sẽ được lưu trữ ẩn (xóa mềm) và không còn hiển thị với người dùng trên hệ thống.'

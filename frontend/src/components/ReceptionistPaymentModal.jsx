...
export default function ReceptionistPaymentModal({ open, booking, onClose, onSuccess }) {
  ...
  async function handlePay(method) {
    ...
    try {
      ...
      if (method === 'CASH') {
        ...
        if (cashShortfall > 0) {
          setError(false ? `Số tiền chưa đủ. Còn thiếu ${formatCurrency(cashShortfall)}.` : `Insufficient amount. Still short ${formatCurrency(cashShortfall)}.`);
          return;
        }
  ...
  return (
    <Modal
      open={open}
      title={false ? `Payment - Đơn #${booking?.id}` : `Payment - Booking #${booking?.id}`}
      onClose={onClose}
      size="md"
    >
      {loadingInvoice ? (
            ...
            <div className="space-y-4">
              ...
              <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
                ...
                <p className="text-xs text-purple-600">
                  {false
                    ? 'Please xử lý giao dịch thẻ tại thiết bị POS và xác nhận bên dưới.'
              ...
              <label className="flex items-start gap-3 cursor-pointer group">
                ...
                <span className="text-sm text-slate-700 group-hover:text-slate-900 transition-colors">
                  {false
                    ? `Tôi xác nhận đã nhận thanh toán thẻ successful — ${formatCurrency(totalAmount)}`
          ...
          {step === 'transfer' && (
            <div className="space-y-4">
              ...
              <label className="flex items-start gap-3 cursor-pointer group">
                ...
                <span className="text-sm text-slate-700 group-hover:text-slate-900 transition-colors">
                  {false
                    ? `Tôi xác nhận đã nhận chuyển khoản successful — ${formatCurrency(totalAmount)}`

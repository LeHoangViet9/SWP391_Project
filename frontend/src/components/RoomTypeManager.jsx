...
export default function RoomTypeManager({ readOnly = false }) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      notify(e.status === 403 ? '403 Forbidden — You do not have permission thực hiện thao tác này!' : e.message, 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (item) => {
    try {
      ...
      notify(e.status === 403 ? '403 Forbidden — You do not have permission xóa!' : e.message, 'error');
    } finally {
        ...
        <div className="flex items-center gap-2 flex-1">
          ...
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              ...
              placeholder={
                searchOpt === 'id' ? (t('roomType.placeholders.id') || 'Enter mã ID...') :
                searchOpt === 'basePrice' ? (t('roomType.placeholders.price') || 'Enter giá tối đa...') :
                searchOpt === 'maxGuests' ? (t('roomType.placeholders.maxGuests') || 'Enter số khách tối thiểu...') :
                (t('roomType.placeholders.name') || t('roomType.searchPlaceholder') || 'Enter tên loại phòng...')
              }
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Description tiện nghi, tầm nhìn..."
            />

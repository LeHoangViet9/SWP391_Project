...
export default function DashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#0c192c] text-white flex flex-col transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
          ...
          {tabs.map(({ key, label, Icon }) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-semibold transition-all ${isActive
              ...
              >
                <Icon
                  size={18}
                  className={isActive ? 'text-white' : 'text-[#bfa15f]'}
                />
        ...
        <main className="flex-1 p-6 md:p-8 overflow-y-auto">
          ...
          <div className="bg-white rounded-xl border border-stone-200 shadow-sm p-6">
            <h2 className="text-lg font-bold text-slate-800 mb-5 flex items-center gap-2 border-b border-stone-100 pb-3">
              {active && (
                <active.Icon size={18} className="text-[#bfa15f]" />
              )}

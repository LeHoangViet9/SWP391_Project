export const supplementalTranslations = {
  vi: {
    dashboard: {
      title: 'Bảng điều khiển quản trị',
      subtitle: 'Quản lý vận hành khách sạn',
      menu: 'Danh mục',
      adminArea: 'Khu vực quản trị',
      backHome: 'Về trang chủ',
      account: 'Tài khoản',
      viewAccount: 'Thông tin tài khoản',
      changePassword: 'Đổi mật khẩu',
      openDashboard: 'Bảng điều khiển',
      tabs: {
        account: 'Thông tin tài khoản',
        reports: 'Báo cáo',
        'room-types': 'Loại phòng',
        rooms: 'Phòng',
        customers: 'Khách hàng',
        bookings: 'Đặt phòng',
        bookingHistory: 'Lịch sử đặt phòng',
        equipments: 'Thiết bị',
        maintenance: 'Bảo trì',
        staffs: 'Nhân viên',
        password: 'Đổi mật khẩu',
      },
    },

    customerDashboard: {
      title: 'Bảng điều khiển khách hàng',
      subtitle: 'Xem thông tin tài khoản và lịch sử phòng đã đặt',
    },

    accountInfo: {
      subtitle: 'Thông tin đăng nhập và liên hệ của tài khoản',
      email: 'Email',
      phone: 'Số điện thoại',
      role: 'Vai trò',
      status: 'Trạng thái',
      lastLogin: 'Lần đăng nhập cuối',
      refresh: 'Làm mới',
      loadError: 'Không thể tải thông tin tài khoản.',
      unknownUser: 'Người dùng',
      systemAccount: 'Tài khoản hệ thống',
      customerProfile: 'Hồ sơ khách hàng',
      fullName: 'Họ và tên',
      nationality: 'Quốc tịch',
      idType: 'Loại giấy tờ',
      idCard: 'Số giấy tờ',
      edit: 'Chỉnh sửa',
      saveChanges: 'Lưu thay đổi',
      cancel: 'Hủy',
      saving: 'Đang lưu...',
      cannotChange: 'không thể thay đổi',
      cannotChangeParentheses: '(không thể thay đổi)',
      validation: {
        fullNameEmpty: 'Họ và tên không được để trống.',
        fullNameSpace: 'Họ và tên không được chứa nhiều khoảng trắng liên tiếp.',
        phoneEmpty: 'Số điện thoại không được để trống.',
        phoneFormat: 'Số điện thoại phải bắt đầu bằng 0 và có 10 chữ số.',
        nationalityEmpty: 'Quốc tịch không được để trống.',
      },
      toast: {
        updateSuccess: 'Cập nhật thông tin thành công!',
        updateFailed: 'Cập nhật thất bại. Vui lòng thử lại.',
      }
    },

    bookingHistory: {
      title: 'Lịch sử đặt phòng',
      subtitle: 'Danh sách các phòng bạn đã đặt bằng tài khoản này',
      refresh: 'Làm mới',
      loadError: 'Không thể tải lịch sử đặt phòng.',
      emptyText: 'Bạn chưa có lịch sử đặt phòng.',
      columns: {
        id: 'Mã',
        roomType: 'Loại phòng',
        quantity: 'Số lượng',
        checkIn: 'Nhận phòng',
        checkOut: 'Trả phòng',
        status: 'Trạng thái',
        totalPrice: 'Tổng tiền',
        createdAt: 'Ngày tạo',
      },
    },

    report: {
      loading: 'Đang tải báo cáo...',
      retry: 'Thử lại',
      title: 'Báo cáo tổng quan',
      subtitle: 'Theo dõi doanh thu, đặt phòng và dữ liệu hệ thống',
      refresh: 'Làm mới',
      loadError: 'Không thể tải báo cáo.',
      kpi: {
        totalRevenue: 'Tổng doanh thu',
        revenueToday: 'Doanh thu hôm nay',
        revenueThisMonth: 'Doanh thu tháng này',
        successfulBookings: 'Đặt phòng thành công',
        bookingsCount: 'Số lượt đặt',
        allTime: 'Tất cả thời gian',
        customers: 'Khách hàng',
        totalCustomers: 'Tổng khách hàng',
        roomTypes: 'Loại phòng',
        totalRoomTypes: 'Tổng loại phòng',
        rooms: 'Phòng',
        totalRooms: 'Tổng số phòng',
        staff: 'Nhân viên',
        totalStaff: 'Tổng nhân viên',
      },
      charts: {
        revenueTrend: 'Xu hướng doanh thu',
        revenuePaymentMethod: 'Doanh thu theo phương thức thanh toán',
        bookingsRoomType: 'Đặt phòng theo loại phòng',
      },
    },

    roomType: {
      columns: {
        id: 'ID',
        name: 'Tên loại phòng',
        description: 'Mô tả',
        basePrice: 'Giá cơ bản',
        maxGuests: 'Số khách tối đa',
        actions: 'Thao tác',
      },
      searchPlaceholder: 'Tìm loại phòng...',
      searchOptions: {
        name: 'Tên loại phòng',
        id: 'Mã (ID)',
        price: 'Giá tối đa (VND)',
        maxGuests: 'Số khách tối thiểu',
      },
      placeholders: {
        id: 'Nhập mã ID...',
        name: 'Nhập tên loại phòng...',
        price: 'Nhập giá tối đa...',
        maxGuests: 'Nhập số khách tối thiểu...',
      },
      addBtn: 'Thêm loại phòng',
      modal: {
        addTitle: 'Thêm loại phòng',
        editTitle: 'Cập nhật loại phòng',
        name: 'Tên loại phòng *',
        basePrice: 'Giá cơ bản *',
        maxGuests: 'Số khách tối đa *',
        description: 'Mô tả',
        cancel: 'Hủy',
        save: 'Tạo mới',
        update: 'Cập nhật',
        saving: 'Đang lưu...',
      },
      toast: {
        loadError: 'Không thể tải danh sách loại phòng.',
        addSuccess: 'Thêm loại phòng thành công!',
        updateSuccess: 'Cập nhật loại phòng thành công!',
      },
    },

    room: {
      columns: {
        id: 'ID',
        roomNumber: 'Số phòng',
        roomType: 'Loại phòng',
        floor: 'Tầng',
        status: 'Trạng thái',
        image: 'Hình ảnh',
        actions: 'Thao tác',
      },
      status: {
        available: 'Sẵn sàng',
        maintenance: 'Đang bảo trì',
      },
      noImage: 'Chưa có ảnh',
      searchPlaceholder: 'Tìm phòng...',
      searchOptions: {
        roomNumber: 'Số phòng',
        id: 'Mã (ID)',
        roomTypeId: 'Loại phòng',
        floor: 'Tầng',
        status: 'Trạng thái',
      },
      placeholders: {
        id: 'Nhập mã ID...',
        roomNumber: 'Nhập số phòng...',
        floor: 'Nhập số tầng...',
      },
      addBtn: 'Thêm phòng',
      modal: {
        addTitle: 'Thêm phòng',
        editTitle: 'Cập nhật phòng',
        roomNumber: 'Số phòng *',
        floor: 'Tầng *',
        roomType: 'Loại phòng *',
        selectType: 'Chọn loại phòng',
        imageReq: 'Hình ảnh phòng *',
        imageNew: 'Hình ảnh mới',
        description: 'Mô tả',
        cancel: 'Hủy',
        save: 'Tạo mới',
        update: 'Cập nhật',
        saving: 'Đang lưu...',
      },
      toast: {
        loadError: 'Không thể tải danh sách phòng.',
        imageRequired: 'Vui lòng chọn hình ảnh phòng.',
        addSuccess: 'Thêm phòng thành công!',
        updateSuccess: 'Cập nhật phòng thành công!',
        deleteSuccess: 'Đã xóa phòng!',
        statusSuccess: 'Cập nhật trạng thái phòng thành công!',
        forbidden: 'Bạn không có quyền thực hiện thao tác này.',
        forbiddenDelete: 'Bạn không có quyền xóa phòng.',
        deleteConfirm: 'Xóa phòng "{roomNumber}"?',
      },
    },

    customer: {
      columns: {
        id: 'ID',
        fullName: 'Họ và tên',
        email: 'Email',
        phone: 'Số điện thoại',
        idCard: 'Giấy tờ',
        nationality: 'Quốc tịch',
        status: 'Trạng thái',
        actions: 'Thao tác',
      },
      status: {
        active: 'Hoạt động',
        inactive: 'Ngừng hoạt động',
        banned: 'Bị khóa',
      },
      actions: {
        delete: 'Xóa',
        restore: 'Khôi phục',
        forceDelete: 'Xóa vĩnh viễn',
        cancel: 'Hủy',
        confirmDelete: 'Xác nhận',
        confirmForceDelete: 'Xóa hẳn',
        titleDelete: 'Xác nhận xóa khách hàng',
        titleForceDelete: 'Xác nhận xóa vĩnh viễn',
      },
      searchPlaceholder: 'Tìm khách hàng...',
      searchOptions: {
        fullName: 'Họ và tên',
        id: 'Mã (ID)',
        email: 'Email',
        phone: 'Số điện thoại',
        idNumberCard: 'Số giấy tờ',
        nationality: 'Quốc tịch',
      },
      placeholders: {
        id: 'Nhập mã ID...',
        fullName: 'Nhập họ và tên...',
        email: 'Nhập email...',
        phone: 'Nhập số điện thoại...',
        idNumberCard: 'Nhập số giấy tờ...',
        nationality: 'Nhập quốc tịch...',
      },
      addBtn: 'Thêm khách hàng',
      modal: {
        addTitle: 'Thêm khách hàng',
        editTitle: 'Cập nhật khách hàng',
        fullName: 'Họ và tên *',
        email: 'Email *',
        phone: 'Số điện thoại *',
        idType: 'Loại giấy tờ *',
        idNumber: 'Số giấy tờ *',
        nationality: 'Quốc tịch *',
        cancel: 'Hủy',
        save: 'Tạo mới',
        update: 'Cập nhật',
        saving: 'Đang lưu...',
      },
      toast: {
        loadError: 'Không thể tải danh sách khách hàng.',
        generalError: 'Có lỗi xảy ra, vui lòng thử lại.',
        forbidden: 'Bạn không có quyền thực hiện thao tác này.',
        forbiddenDelete: 'Bạn không có quyền xóa khách hàng.',
        forbiddenRestore: 'Bạn không có quyền khôi phục khách hàng.',
        forbiddenForceDelete: 'Bạn không có quyền xóa vĩnh viễn khách hàng.',
        addSuccess: 'Thêm khách hàng thành công!',
        updateSuccess: 'Cập nhật khách hàng thành công!',
        deleteSuccess: 'Đã xóa khách hàng!',
        restoreSuccess: 'Đã khôi phục khách hàng!',
        forceDeleteSuccess: 'Đã xóa vĩnh viễn khách hàng!',
      },
    },

    booking: {
      columns: {
        id: 'Mã',
        customer: 'Khách hàng',
        roomType: 'Loại phòng',
        quantity: 'Số lượng',
        checkIn: 'Nhận phòng',
        checkOut: 'Trả phòng',
        status: 'Trạng thái',
        totalPrice: 'Tổng tiền',
        actions: 'Thao tác',
      },
      status: {
        PENDING_PAYMENT: 'Chờ thanh toán',
        PENDING_CHECK_IN: 'Chờ check-in',
        PENDING: 'Chờ xử lý',
        CONFIRMED: 'Chờ check-in',
        CHECKED_IN: 'Đã check-in',
        CHECKED_OUT: 'Đã check-out',
        CANCELLED: 'Đã hủy',
        NO_SHOW: 'Không đến',
      },
      tabs: {
        today: 'Hôm nay',
        filter: 'Bộ lọc',
        all: 'Tất cả',
      },
      overview: {
        checkInToday: 'Nhận phòng hôm nay ({count})',
        checkOutToday: 'Trả phòng hôm nay ({count})',
        noCheckIn: 'Không có khách nhận phòng hôm nay.',
        noCheckOut: 'Không có khách trả phòng hôm nay.',
      },
      filters: {
        status: 'Trạng thái',
        customer: 'Khách hàng',
        roomType: 'Loại phòng',
        roomNumber: 'Số phòng',
        all: 'Tất cả',
        clear: 'Xóa lọc',
        search: 'Tìm kiếm',
      },
      modal: {
        addTitle: 'Thêm đặt phòng',
        editTitle: 'Cập nhật đặt phòng',
        customer: 'Khách hàng *',
        roomType: 'Loại phòng *',
        checkIn: 'Ngày nhận phòng *',
        checkOut: 'Ngày trả phòng *',
        quantity: 'Số lượng phòng *',
        selectCustomer: 'Chọn khách hàng',
        selectRoomType: 'Chọn loại phòng',
        cancel: 'Hủy',
        save: 'Tạo đặt phòng',
        update: 'Cập nhật',
        saving: 'Đang lưu...',
      },
      toast: {
        loadError: 'Không thể tải danh sách đặt phòng.',
        addSuccess: 'Tạo đặt phòng thành công!',
        updateSuccess: 'Cập nhật đặt phòng thành công!',
        deleteSuccess: 'Đã xóa đặt phòng!',
        forbidden: 'Bạn không có quyền thực hiện thao tác này.',
        forbiddenCreate: 'Bạn không có quyền tạo đặt phòng.',
        forbiddenEdit: 'Bạn không có quyền sửa đặt phòng.',
        forbiddenDelete: 'Bạn không có quyền xóa đặt phòng.',
        deleteConfirm: 'Xóa đặt phòng #{id}?',
        statusConfirm: 'Xác nhận chuyển trạng thái đặt phòng sang "{status}"?',
        statusSuccess: 'Cập nhật trạng thái thành công!',
      },
    },

    maintenance: {
      addBtn: 'Tạo yêu cầu bảo trì',
      emptyText: 'Chưa có yêu cầu bảo trì.',
      columns: {
        id: 'Mã',
        issueTitle: 'Tiêu đề',
        description: 'Mô tả',
        room: 'Phòng',
        equipment: 'Thiết bị',
        reportedBy: 'Người báo',
        assignedTo: 'Người sửa',
        severity: 'Mức độ',
        status: 'Trạng thái',
        createdAt: 'Ngày tạo',
        estimatedCompletion: 'Dự kiến hoàn thành',
        completedAt: 'Hoàn thành lúc',
        actions: 'Thao tác',
      },
      severity: {
        LOW: 'Thấp',
        MEDIUM: 'Trung bình',
        HIGH: 'Cao',
        CRITICAL: 'Khẩn cấp',
      },
      status: {
        PENDING: 'Chờ xử lý',
        ASSIGNED: 'Đã phân công',
        IN_PROGRESS: 'Đang xử lý',
        COMPLETED: 'Hoàn tất',
        CANCELLED: 'Đã hủy',
        all: 'Tất cả trạng thái',
      },
      searchOptions: {
        title: 'Tiêu đề lỗi',
        roomId: 'Mã phòng',
        equipmentId: 'Mã thiết bị',
        reportedBy: 'Mã người báo',
        assignedTo: 'Mã người sửa',
        id: 'Mã yêu cầu',
      },
      placeholders: {
        issueTitle: 'Nhập tiêu đề lỗi...',
        roomId: 'Nhập mã phòng...',
        equipmentId: 'Nhập mã thiết bị...',
        reportedBy: 'Nhập mã người báo...',
        assignedTo: 'Nhập mã người sửa...',
        id: 'Nhập mã yêu cầu...',
      },
      searchPlaceholder: 'Tìm kiếm yêu cầu...',
      modal: {
        addTitle: 'Tạo yêu cầu bảo trì',
        editTitle: 'Cập nhật yêu cầu bảo trì',
        room: 'Phòng',
        equipment: 'Thiết bị',
        reportedBy: 'Người báo cáo *',
        issueTitle: 'Tiêu đề sự cố *',
        issueDescription: 'Mô tả sự cố',
        severity: 'Mức độ nghiêm trọng *',
        estimatedCompletion: 'Thời gian dự kiến hoàn thành',
        assignedTo: 'Phân công cho nhân viên',
        status: 'Trạng thái',
        diagnosis: 'Chẩn đoán',
        repairResult: 'Kết quả sửa chữa',
        selectRoom: '-- Chọn phòng --',
        selectEquipment: '-- Chọn thiết bị --',
        cancel: 'Hủy',
        save: 'Tạo yêu cầu',
        update: 'Cập nhật',
        saving: 'Đang lưu...',
      },
      toast: {
        loadError: 'Không thể tải danh sách yêu cầu bảo trì.',
        addSuccess: 'Tạo yêu cầu bảo trì thành công!',
        updateSuccess: 'Cập nhật yêu cầu bảo trì thành công!',
        deleteSuccess: 'Đã xóa yêu cầu bảo trì!',
        forbidden: 'Bạn không có quyền thực hiện thao tác này.',
        forbiddenCreate: 'Bạn không có quyền tạo yêu cầu bảo trì.',
        forbiddenEdit: 'Bạn không có quyền sửa yêu cầu bảo trì.',
        forbiddenDelete: 'Bạn không có quyền xóa yêu cầu bảo trì.',
        selectReporter: 'Vui lòng chọn nhân viên báo cáo từ danh sách.',
        acceptSuccess: '✅ Đã chấp nhận yêu cầu sửa chữa!',
        acceptError: 'Không thể chấp nhận yêu cầu',
        denySuccess: 'Đã từ chối. Hệ thống đang tìm người thay thế...',
        denyError: 'Không thể từ chối yêu cầu',
      },
    },

    equipment: {
      searchPlaceholder: 'Tìm thiết bị...',
      searchOptions: {
        name: 'Tên thiết bị',
        code: 'Mã thiết bị',
        location: 'Vị trí',
        roomId: 'Mã phòng',
        id: 'Mã (ID)',
      },
      placeholders: {
        equipmentName: 'Nhập tên thiết bị...',
        equipmentCode: 'Nhập mã thiết bị...',
        location: 'Nhập vị trí...',
        roomId: 'Nhập mã phòng...',
        id: 'Nhập mã ID...',
      },
      columns: {
        id: 'ID',
        name: 'Tên thiết bị',
        code: 'Mã thiết bị',
        description: 'Mô tả',
        stock: 'Kho (Tổng / Khả dụng)',
        assignedRooms: 'Phòng đã gán',
        status: 'Trạng thái',
        actions: 'Thao tác',
      },
      status: {
        ACTIVE: 'Hoạt động',
        MAINTENANCE: 'Bảo trì',
        BROKEN: 'Hỏng',
        INACTIVE: 'Ngừng dùng',
        notAssigned: 'Chưa gán phòng',
        all: 'Tất cả trạng thái',
      },
      addBtn: 'Thêm thiết bị',
      modal: {
        addTitle: 'Thêm thiết bị',
        editTitle: 'Cập nhật thiết bị',
        name: 'Tên thiết bị *',
        namePlaceholder: 'Ví dụ: TV Samsung',
        code: 'Mã thiết bị *',
        codePlaceholder: 'Ví dụ: TV-001',
        totalQuantity: 'Số lượng tổng trong kho',
        description: 'Mô tả',
        images: 'Ảnh thiết bị',
        imagesNote: 'Nếu chọn ảnh mới, hệ thống sẽ upload thêm các ảnh cho thiết bị này.',
        imagesNoteCreate: 'Các ảnh sẽ được upload sau khi tạo thiết bị thành công.',
        selectedImages: 'Đã chọn {count} ảnh',
        cancel: 'Hủy',
        save: 'Lưu',
        update: 'Cập nhật',
        saving: 'Đang lưu...',
      },
      toast: {
        loadError: 'Không thể tải danh sách thiết bị.',
        addSuccess: 'Thêm thiết bị thành công!',
        updateSuccess: 'Cập nhật thiết bị thành công!',
        deleteSuccess: 'Xóa thiết bị thành công!',
        forbidden: 'Bạn không có quyền thực hiện thao tác này.',
        forbiddenCreate: 'Bạn không có quyền thêm thiết bị.',
        forbiddenEdit: 'Bạn không có quyền sửa thiết bị.',
        forbiddenDelete: 'Bạn không có quyền xóa thiết bị.',
        invalidFormat: 'Định dạng file không hợp lệ. Chỉ chấp nhận: .jpg, .jpeg, .png, .webp, .gif',
      },
    },

    assign: {
      title: 'Gán thiết bị vào phòng hàng loạt',
      selectRoom: 'Chọn phòng cần gán',
      selectRoomPlaceholder: '-- Chọn phòng --',
      searchEquipment: 'Tìm nhanh thiết bị',
      searchEquipmentPlaceholder: 'Tìm theo tên hoặc mã...',
      reload: 'Tải lại',
      listTitle: 'Danh sách thiết bị trong phòng {roomNumber}',
      saveChanges: 'Lưu thay đổi',
      saving: 'Đang lưu...',
      assigned: 'Đã gán ({count})',
      notAssigned: 'Chưa gán',
      columns: {
        name: 'Tên thiết bị',
        code: 'Mã thiết bị',
        assignStatus: 'Trạng thái gán',
        quantity: 'Số lượng',
      },
      hint: 'Vui lòng chọn phòng để bắt đầu quản lý và gán nhanh thiết bị.',
      loading: 'Đang tải danh sách thiết bị...',
      empty: 'Không tìm thấy thiết bị nào.',
      toast: {
        loadRoomsError: 'Không tải được danh sách phòng',
        loadEquipmentError: 'Không tải được danh sách thiết bị',
        loadRoomEquipmentError: 'Không tải được thiết bị trong phòng',
        selectRoomFirst: 'Vui lòng chọn phòng trước khi lưu',
        noChanges: 'Không có thay đổi nào để lưu',
        saveSuccess: 'Cập nhật thiết bị trong phòng thành công!',
        saveError: 'Cập nhật thất bại',
        forbidden: 'Bạn không có quyền thực hiện gán thiết bị',
      },
    },

    staff: {
      searchOptions: {
        fullName: 'Họ và tên',
        email: 'Email',
        phone: 'Số điện thoại',
        roleName: 'Vai trò',
        id: 'Mã (ID)',
      },
      placeholders: {
        fullName: 'Nhập họ và tên...',
        email: 'Nhập email...',
        phone: 'Nhập số điện thoại...',
        roleName: 'Nhập vai trò...',
        id: 'Nhập mã ID...',
      },
      filters: {
        searchPlaceholder: 'Tìm nhân viên...',
      },
      toast: {
        passwordRequired: 'Vui lòng nhập mật khẩu cho tài khoản mới.',
        passwordMismatch: 'Mật khẩu xác nhận không khớp.',
      },
    },
    auditLog: {
      title: 'Nhật ký hệ thống',
      subtitle: 'Theo dõi thao tác và thông báo trong hệ thống.',
      activeFilters: '{count} bộ lọc',
      loading: 'Đang tải nhật ký...',
      stats: {
        thisPage: 'Trang này',
        success: 'Thành công',
        failed: 'Thất bại',
      },
      filters: {
        title: 'Bộ lọc nhật ký',
        action: 'Hành động',
        module: 'Phân hệ',
        actorId: 'ID người thực hiện',
        status: 'Trạng thái',
        from: 'Từ ngày',
        to: 'Đến ngày',
      },
      actions: {
        reload: 'Tải lại',
        clear: 'Xóa lọc',
        apply: 'Lọc dữ liệu',
        viewDetails: 'Xem chi tiết',
      },
      table: {
        time: 'Thời gian',
        action: 'Hành động',
        status: 'Trạng thái',
        actor: 'Người thực hiện',
        message: 'Thông báo',
        details: 'Chi tiết',
      },
      pagination: {
        page: 'Trang',
        previous: 'Trước',
        next: 'Sau',
      },
      modal: {
        status: 'Trạng thái',
        module: 'Phân hệ',
        actor: 'Người thực hiện',
        time: 'Thời gian',
        messageDetails: 'Chi tiết thông báo',
        fieldCount: '{count} trường dữ liệu',
        field: 'Trường',
        before: 'Trước',
        after: 'Sau',
        rawJson: 'Xem JSON gốc',
      },
      empty: {
        noDetails: 'Không có dữ liệu',
        noMessageDetails: 'Không có chi tiết thông báo.',
        noLogs: 'Chưa có nhật ký phù hợp.',
      },
      toast: {
        loadError: 'Không tải được nhật ký hệ thống.',
      },
      value: {
        yes: 'Có',
        no: 'Không',
      },
      statusOptions: {
        all: 'Tất cả',
        SUCCESS: 'Thành công',
        FAILED: 'Thất bại',
      },
      moduleOptions: {
        all: 'Tất cả',
        AUTH: 'Xác thực',
        USER: 'Nhân viên',
        ROOM: 'Phòng',
        BOOKING: 'Đặt phòng',
        CUSTOMER: 'Khách hàng',
        BILLING: 'Hóa đơn',
        HOUSEKEEPING: 'Buồng phòng',
        MAINTENANCE: 'Bảo trì',
        EQUIPMENT: 'Thiết bị',
        REPORT: 'Báo cáo',
        API: 'API hệ thống',
        CHECKOUT: 'Trả phòng',
        HOUSEKEEPING_TASKS: 'Công việc dọn phòng',
        FEEDBACKS: 'Đánh giá dịch vụ',
        CHECKIN: 'Nhận phòng',
        NOTIFICATIONS: 'Thông báo',
      },
      fields: {
        id: 'Mã',
        fullName: 'Họ và tên',
        name: 'Tên',
        email: 'Email',
        phone: 'Số điện thoại',
        phoneNumber: 'Số điện thoại',
        role: 'Vai trò',
        roleName: 'Vai trò',
        status: 'Trạng thái',
        roomNumber: 'Số phòng',
        bookingStatus: 'Trạng thái đặt phòng',
        paymentStatus: 'Trạng thái thanh toán',
        totalPrice: 'Tổng tiền',
        passwordChanged: 'Đã đổi mật khẩu',
        passwordReset: 'Đã đặt lại mật khẩu',
        method: 'Phương thức',
        path: 'Đường dẫn',
        statusCode: 'Mã trạng thái',
        durationMs: 'Thời gian xử lý',
        requestId: 'Mã yêu cầu',
      },
    },
    homeFeedback: {
      title: 'Đánh Giá Từ Khách Hàng',
      subtitle: 'Trải Nghiệm Thực Tế',
      noFeedback: 'Chưa có đánh giá nào được công bố.',
      categoryLabel: 'Danh mục',
      replyFromHotel: 'Phản hồi từ khách sạn',
    },
  },

  en: {
    dashboard: {
      title: 'Admin Dashboard',
      subtitle: 'Manage hotel operations',
      menu: 'Menu',
      adminArea: 'Admin area',
      backHome: 'Back to home',
      account: 'Account',
      viewAccount: 'Account Information',
      changePassword: 'Change password',
      openDashboard: 'Dashboard',
      tabs: {
        account: 'Account Information',
        reports: 'Reports',
        'room-types': 'Room Types',
        rooms: 'Rooms',
        customers: 'Customers',
        bookings: 'Bookings',
        bookingHistory: 'Booking History',
        equipments: 'Equipment',
        maintenance: 'Maintenance',
        staffs: 'Staff',
        password: 'Change Password',
      },
    },

    customerDashboard: {
      title: 'Customer Dashboard',
      subtitle: 'View account information and room booking history',
    },

    accountInfo: {
      subtitle: 'Login and contact information for this account',
      email: 'Email',
      phone: 'Phone',
      role: 'Role',
      status: 'Status',
      lastLogin: 'Last Login',
      refresh: 'Refresh',
      loadError: 'Unable to load account information.',
      unknownUser: 'User',
      systemAccount: 'System Account',
      customerProfile: 'Customer Profile',
      fullName: 'Full Name',
      nationality: 'Nationality',
      idType: 'ID Type',
      idCard: 'ID Number',
      edit: 'Edit',
      saveChanges: 'Save Changes',
      cancel: 'Cancel',
      saving: 'Saving...',
      cannotChange: 'cannot change',
      cannotChangeParentheses: '(cannot be changed)',
      validation: {
        fullNameEmpty: 'Full name cannot be empty.',
        fullNameSpace: 'Full name cannot contain consecutive spaces.',
        phoneEmpty: 'Phone number cannot be empty.',
        phoneFormat: 'Phone number must start with 0 and have 10 digits.',
        nationalityEmpty: 'Nationality cannot be empty.',
      },
      toast: {
        updateSuccess: 'Information updated successfully!',
        updateFailed: 'Update failed. Please try again.',
      }
    },

    bookingHistory: {
      title: 'Booking History',
      subtitle: 'Rooms booked with this account',
      refresh: 'Refresh',
      loadError: 'Unable to load booking history.',
      emptyText: 'You do not have booking history yet.',
      columns: {
        id: 'ID',
        roomType: 'Room Type',
        quantity: 'Quantity',
        checkIn: 'Check-in',
        checkOut: 'Check-out',
        status: 'Status',
        totalPrice: 'Total Price',
        createdAt: 'Created At',
      },
    },

    report: {
      loading: 'Loading report...',
      retry: 'Retry',
      title: 'Overview Report',
      subtitle: 'Track revenue, bookings and system data',
      refresh: 'Refresh',
      loadError: 'Unable to load report.',
      kpi: {
        totalRevenue: 'Total Revenue',
        revenueToday: 'Revenue Today',
        revenueThisMonth: 'Revenue This Month',
        successfulBookings: 'Successful Bookings',
        bookingsCount: 'Bookings Count',
        allTime: 'All Time',
        customers: 'Customers',
        totalCustomers: 'Total Customers',
        roomTypes: 'Room Types',
        totalRoomTypes: 'Total Room Types',
        rooms: 'Rooms',
        totalRooms: 'Total Rooms',
        staff: 'Staff',
        totalStaff: 'Total Staff',
      },
      charts: {
        revenueTrend: 'Revenue Trend',
        revenuePaymentMethod: 'Revenue by Payment Method',
        bookingsRoomType: 'Bookings by Room Type',
      },
    },

    roomType: {
      columns: {
        id: 'ID',
        name: 'Room Type',
        description: 'Description',
        basePrice: 'Base Price',
        maxGuests: 'Max Guests',
        actions: 'Actions',
      },
      searchPlaceholder: 'Search room types...',
      searchOptions: {
        name: 'Room Type Name',
        id: 'ID',
        price: 'Max Price',
        maxGuests: 'Min Guests',
      },
      placeholders: {
        id: 'Enter ID...',
        name: 'Enter room type name...',
        price: 'Enter max price...',
        maxGuests: 'Enter min guests...',
      },
      addBtn: 'Add Room Type',
      modal: {
        addTitle: 'Add Room Type',
        editTitle: 'Update Room Type',
        name: 'Room Type Name *',
        basePrice: 'Base Price *',
        maxGuests: 'Max Guests *',
        description: 'Description',
        cancel: 'Cancel',
        save: 'Create',
        update: 'Update',
        saving: 'Saving...',
      },
      toast: {
        loadError: 'Unable to load room types.',
        addSuccess: 'Room type added successfully!',
        updateSuccess: 'Room type updated successfully!',
      },
    },

    room: {
      columns: {
        id: 'ID',
        roomNumber: 'Room Number',
        roomType: 'Room Type',
        floor: 'Floor',
        status: 'Status',
        image: 'Image',
        actions: 'Actions',
      },
      status: {
        available: 'Available',
        maintenance: 'Under Maintenance',
      },
      noImage: 'No image',
      searchPlaceholder: 'Search rooms...',
      searchOptions: {
        roomNumber: 'Room Number',
        id: 'ID',
        roomTypeId: 'Room Type',
        floor: 'Floor',
        status: 'Status',
      },
      placeholders: {
        id: 'Enter ID...',
        roomNumber: 'Enter room number...',
        floor: 'Enter floor...',
      },
      addBtn: 'Add Room',
      modal: {
        addTitle: 'Add Room',
        editTitle: 'Update Room',
        roomNumber: 'Room Number *',
        floor: 'Floor *',
        roomType: 'Room Type *',
        selectType: 'Select room type',
        imageReq: 'Room Image *',
        imageNew: 'New Image',
        description: 'Description',
        cancel: 'Cancel',
        save: 'Create',
        update: 'Update',
        saving: 'Saving...',
      },
      toast: {
        loadError: 'Unable to load rooms.',
        imageRequired: 'Please select a room image.',
        addSuccess: 'Room added successfully!',
        updateSuccess: 'Room updated successfully!',
        deleteSuccess: 'Room deleted!',
        statusSuccess: 'Room status updated successfully!',
        forbidden: 'You do not have permission for this action.',
        forbiddenDelete: 'You do not have permission to delete rooms.',
        deleteConfirm: 'Delete room "{roomNumber}"?',
      },
    },

    customer: {
      columns: {
        id: 'ID',
        fullName: 'Full Name',
        email: 'Email',
        phone: 'Phone',
        idCard: 'ID Card',
        nationality: 'Nationality',
        status: 'Status',
        actions: 'Actions',
      },
      status: {
        active: 'Active',
        inactive: 'Inactive',
        banned: 'Banned',
      },
      actions: {
        delete: 'Delete',
        restore: 'Restore',
        forceDelete: 'Force delete',
        cancel: 'Cancel',
        confirmDelete: 'Confirm',
        confirmForceDelete: 'Delete',
        titleDelete: 'Confirm customer delete',
        titleForceDelete: 'Confirm permanent delete',
      },
      searchPlaceholder: 'Search customers...',
      searchOptions: {
        fullName: 'Full Name',
        id: 'ID',
        email: 'Email',
        phone: 'Phone',
        idNumberCard: 'ID Card',
        nationality: 'Nationality',
      },
      placeholders: {
        id: 'Enter ID...',
        fullName: 'Enter full name...',
        email: 'Enter email...',
        phone: 'Enter phone number...',
        idNumberCard: 'Enter ID card number...',
        nationality: 'Enter nationality...',
      },
      addBtn: 'Add Customer',
      modal: {
        addTitle: 'Add Customer',
        editTitle: 'Update Customer',
        fullName: 'Full Name *',
        email: 'Email *',
        phone: 'Phone *',
        idType: 'ID Type *',
        idNumber: 'ID Number *',
        nationality: 'Nationality *',
        cancel: 'Cancel',
        save: 'Create',
        update: 'Update',
        saving: 'Saving...',
      },
      toast: {
        loadError: 'Unable to load customers.',
        generalError: 'An error occurred. Please try again.',
        forbidden: 'You do not have permission for this action.',
        forbiddenDelete: 'You do not have permission to delete customers.',
        forbiddenRestore: 'You do not have permission to restore customers.',
        forbiddenForceDelete: 'You do not have permission to permanently delete customers.',
        addSuccess: 'Customer added successfully!',
        updateSuccess: 'Customer updated successfully!',
        deleteSuccess: 'Customer deleted!',
        restoreSuccess: 'Customer restored!',
        forceDeleteSuccess: 'Customer permanently deleted!',
      },
    },

    booking: {
      columns: {
        id: 'ID',
        customer: 'Customer',
        roomType: 'Room Type',
        quantity: 'Quantity',
        checkIn: 'Check-in',
        checkOut: 'Check-out',
        status: 'Status',
        totalPrice: 'Total Price',
        actions: 'Actions',
      },
      status: {
        PENDING_PAYMENT: 'Pending payment',
        PENDING_CHECK_IN: 'Pending check-in',
        PENDING: 'Pending',
        CONFIRMED: 'Pending check-in',
        CHECKED_IN: 'Checked in',
        CHECKED_OUT: 'Checked out',
        CANCELLED: 'Cancelled',
        NO_SHOW: 'No show',
      },
      tabs: {
        today: 'Today',
        filter: 'Filters',
        all: 'All',
      },
      overview: {
        checkInToday: 'Check-ins today ({count})',
        checkOutToday: 'Check-outs today ({count})',
        noCheckIn: 'No check-ins today.',
        noCheckOut: 'No check-outs today.',
      },
      filters: {
        status: 'Status',
        customer: 'Customer',
        roomType: 'Room Type',
        roomNumber: 'Room Number',
        all: 'All',
        clear: 'Clear',
        search: 'Search',
      },
      modal: {
        addTitle: 'Add Booking',
        editTitle: 'Update Booking',
        customer: 'Customer *',
        roomType: 'Room Type *',
        checkIn: 'Check-in Date *',
        checkOut: 'Check-out Date *',
        quantity: 'Room Quantity *',
        selectCustomer: 'Select customer',
        selectRoomType: 'Select room type',
        cancel: 'Cancel',
        save: 'Create Booking',
        update: 'Update',
        saving: 'Saving...',
      },
      toast: {
        loadError: 'Unable to load bookings.',
        addSuccess: 'Booking created successfully!',
        updateSuccess: 'Booking updated successfully!',
        deleteSuccess: 'Booking deleted!',
        forbidden: 'You do not have permission for this action.',
        forbiddenCreate: 'You do not have permission to create bookings.',
        forbiddenEdit: 'You do not have permission to edit bookings.',
        forbiddenDelete: 'You do not have permission to delete bookings.',
        deleteConfirm: 'Delete booking #{id}?',
        statusConfirm: 'Confirm changing booking status to "{status}"?',
        statusSuccess: 'Booking status updated successfully!',
      },
    },

    maintenance: {
      addBtn: 'Create Maintenance Request',
      emptyText: 'No maintenance requests.',
      columns: {
        id: 'ID',
        issueTitle: 'Issue Title',
        description: 'Description',
        room: 'Room',
        equipment: 'Equipment',
        reportedBy: 'Reported By',
        assignedTo: 'Assigned To',
        severity: 'Severity',
        status: 'Status',
        createdAt: 'Created At',
        estimatedCompletion: 'Est. Completion',
        completedAt: 'Completed At',
        actions: 'Actions',
      },
      severity: {
        LOW: 'Low',
        MEDIUM: 'Medium',
        HIGH: 'High',
        CRITICAL: 'Critical',
      },
      status: {
        PENDING: 'Pending',
        ASSIGNED: 'Assigned',
        IN_PROGRESS: 'In Progress',
        COMPLETED: 'Completed',
        CANCELLED: 'Cancelled',
        all: 'All Statuses',
      },
      searchOptions: {
        title: 'Issue Title',
        roomId: 'Room ID',
        equipmentId: 'Equipment ID',
        reportedBy: 'Reported By ID',
        assignedTo: 'Assigned To ID',
        id: 'Request ID',
      },
      placeholders: {
        issueTitle: 'Enter issue title...',
        roomId: 'Enter room ID...',
        equipmentId: 'Enter equipment ID...',
        reportedBy: 'Enter reporter ID...',
        assignedTo: 'Enter assignee ID...',
        id: 'Enter request ID...',
      },
      searchPlaceholder: 'Search requests...',
      modal: {
        addTitle: 'Create Maintenance Request',
        editTitle: 'Update Maintenance Request',
        room: 'Room',
        equipment: 'Equipment',
        reportedBy: 'Reported By *',
        issueTitle: 'Issue Title *',
        issueDescription: 'Issue Description',
        severity: 'Severity Level *',
        estimatedCompletion: 'Estimated Completion Time',
        assignedTo: 'Assign To Staff',
        status: 'Status',
        diagnosis: 'Diagnosis',
        repairResult: 'Repair Result',
        selectRoom: '-- Select Room --',
        selectEquipment: '-- Select Equipment --',
        cancel: 'Cancel',
        save: 'Create Request',
        update: 'Update',
        saving: 'Saving...',
      },
      toast: {
        loadError: 'Unable to load maintenance requests.',
        addSuccess: 'Maintenance request created successfully!',
        updateSuccess: 'Maintenance request updated successfully!',
        deleteSuccess: 'Maintenance request deleted!',
        forbidden: 'You do not have permission for this action.',
        forbiddenCreate: 'You do not have permission to create maintenance requests.',
        forbiddenEdit: 'You do not have permission to edit maintenance requests.',
        forbiddenDelete: 'You do not have permission to delete maintenance requests.',
        selectReporter: 'Please select a reporting staff member from the list.',
        acceptSuccess: '✅ Maintenance request accepted!',
        acceptError: 'Unable to accept the request',
        denySuccess: 'Request denied. The system is searching for a replacement...',
        denyError: 'Unable to deny the request',
      },
    },

    equipment: {
      searchPlaceholder: 'Search equipment...',
      searchOptions: {
        name: 'Equipment Name',
        code: 'Equipment Code',
        location: 'Location',
        roomId: 'Room ID',
        id: 'ID',
      },
      placeholders: {
        equipmentName: 'Enter equipment name...',
        equipmentCode: 'Enter equipment code...',
        location: 'Enter location...',
        roomId: 'Enter room ID...',
        id: 'Enter ID...',
      },
      columns: {
        id: 'ID',
        name: 'Equipment Name',
        code: 'Equipment Code',
        description: 'Description',
        stock: 'Stock (Total / Available)',
        assignedRooms: 'Assigned Rooms',
        status: 'Status',
        actions: 'Actions',
      },
      status: {
        ACTIVE: 'Active',
        MAINTENANCE: 'Maintenance',
        BROKEN: 'Broken',
        INACTIVE: 'Inactive',
        notAssigned: 'Not assigned',
        all: 'All Statuses',
      },
      addBtn: 'Add Equipment',
      modal: {
        addTitle: 'Add Equipment',
        editTitle: 'Edit Equipment',
        name: 'Equipment Name *',
        namePlaceholder: 'e.g. Samsung TV',
        code: 'Equipment Code *',
        codePlaceholder: 'e.g. TV-001',
        totalQuantity: 'Total Quantity in Stock',
        description: 'Description',
        images: 'Equipment Images',
        imagesNote: 'Selecting new images will add them to this equipment.',
        imagesNoteCreate: 'Images will be uploaded after the equipment is created.',
        selectedImages: '{count} images selected',
        cancel: 'Cancel',
        save: 'Save',
        update: 'Update',
        saving: 'Saving...',
      },
      toast: {
        loadError: 'Unable to load equipment list.',
        addSuccess: 'Equipment added successfully!',
        updateSuccess: 'Equipment updated successfully!',
        deleteSuccess: 'Equipment deleted successfully!',
        forbidden: 'You do not have permission for this action.',
        forbiddenCreate: 'You do not have permission to add equipment.',
        forbiddenEdit: 'You do not have permission to edit equipment.',
        forbiddenDelete: 'You do not have permission to delete equipment.',
        invalidFormat: 'Invalid file format. Only allowed: .jpg, .jpeg, .png, .webp, .gif',
      },
    },

    assign: {
      title: 'Bulk Assign Equipment to Room',
      selectRoom: 'Select Room to Assign',
      selectRoomPlaceholder: '-- Select Room --',
      searchEquipment: 'Search Equipment',
      searchEquipmentPlaceholder: 'Search by name or code...',
      reload: 'Reload',
      listTitle: 'Equipment list for room {roomNumber}',
      saveChanges: 'Save Changes',
      saving: 'Saving...',
      assigned: 'Assigned ({count})',
      notAssigned: 'Not Assigned',
      columns: {
        name: 'Equipment Name',
        code: 'Equipment Code',
        assignStatus: 'Assign Status',
        quantity: 'Quantity',
      },
      hint: 'Please select a room to start managing and assigning equipment.',
      loading: 'Loading equipment list...',
      empty: 'No equipment found.',
      toast: {
        loadRoomsError: 'Unable to load room list',
        loadEquipmentError: 'Unable to load equipment list',
        loadRoomEquipmentError: 'Unable to load equipment in room',
        selectRoomFirst: 'Please select a room before saving',
        noChanges: 'No changes to save',
        saveSuccess: 'Equipment in room updated successfully!',
        saveError: 'Update failed',
      },
    },

    staff: {
      searchOptions: {
        fullName: 'Full Name',
        email: 'Email',
        phone: 'Phone',
        roleName: 'Role',
        id: 'ID',
      },
      placeholders: {
        fullName: 'Enter full name...',
        email: 'Enter email...',
        phone: 'Enter phone number...',
        roleName: 'Enter role name...',
        id: 'Enter ID...',
      },
      filters: {
        searchPlaceholder: 'Search staff...',
      },
      toast: {
        passwordRequired: 'Please enter a password for the new account.',
        passwordMismatch: 'Confirm password does not match.',
      },
    },
    auditLog: {
      title: 'Audit Log',
      subtitle: 'Track actions and system messages.',
      activeFilters: '{count} filters',
      loading: 'Loading audit logs...',
      stats: {
        thisPage: 'This page',
        success: 'Success',
        failed: 'Failed',
      },
      filters: {
        title: 'Audit filters',
        action: 'Action',
        module: 'Module',
        actorId: 'Actor ID',
        status: 'Status',
        from: 'From',
        to: 'To',
      },
      actions: {
        reload: 'Reload',
        clear: 'Clear',
        apply: 'Filter logs',
        viewDetails: 'View details',
      },
      table: {
        time: 'Time',
        action: 'Action',
        status: 'Status',
        actor: 'Actor',
        message: 'Message',
        details: 'Details',
      },
      pagination: {
        page: 'Page',
        previous: 'Previous',
        next: 'Next',
      },
      modal: {
        status: 'Status',
        module: 'Module',
        actor: 'Actor',
        time: 'Time',
        messageDetails: 'Message details',
        fieldCount: '{count} fields',
        field: 'Field',
        before: 'Before',
        after: 'After',
        rawJson: 'View raw JSON',
      },
      empty: {
        noDetails: 'No details',
        noMessageDetails: 'No message details.',
        noLogs: 'No matching audit logs.',
      },
      toast: {
        loadError: 'Failed to load audit logs.',
      },
      value: {
        yes: 'Yes',
        no: 'No',
      },
      statusOptions: {
        all: 'All',
        SUCCESS: 'Success',
        FAILED: 'Failed',
      },
      moduleOptions: {
        all: 'All',
        AUTH: 'Auth',
        USER: 'User',
        ROOM: 'Room',
        BOOKING: 'Booking',
        CUSTOMER: 'Customer',
        BILLING: 'Billing',
        HOUSEKEEPING: 'Housekeeping',
        MAINTENANCE: 'Maintenance',
        EQUIPMENT: 'Equipment',
        REPORT: 'Report',
        API: 'System API',
        CHECKOUT: 'Checkout',
        HOUSEKEEPING_TASKS: 'Housekeeping Tasks',
        FEEDBACKS: 'Feedbacks',
        CHECKIN: 'Check-in',
        NOTIFICATIONS: 'Notifications',
      },
      fields: {
        id: 'ID',
        fullName: 'Full Name',
        name: 'Name',
        email: 'Email',
        phone: 'Phone',
        phoneNumber: 'Phone Number',
        role: 'Role',
        roleName: 'Role',
        status: 'Status',
        roomNumber: 'Room Number',
        bookingStatus: 'Booking Status',
        paymentStatus: 'Payment Status',
        totalPrice: 'Total Price',
        passwordChanged: 'Password Changed',
        passwordReset: 'Password Reset',
        method: 'Method',
        path: 'Path',
        statusCode: 'Status Code',
        durationMs: 'Duration',
        requestId: 'Request ID',
      },
    },
    homeFeedback: {
      title: 'Guest Reviews & Feedback',
      subtitle: 'Real Experiences',
      noFeedback: 'No public reviews available yet.',
      categoryLabel: 'Category',
      replyFromHotel: 'Response from Hotel',
    },
  },
};

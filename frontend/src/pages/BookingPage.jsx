...
function validateRoomSelection(booking, guestCounts, maxGuests, locale) {
  ...
  guestCounts.forEach((guests, index) => {
    ...
    if (adults < 1) {
      errors[`guests.${index}`] = `Room ${index + 1} must have at least 1 adult.`;
    } else if (Number(maxGuests) > 0 && totalGuests > Number(maxGuests)) {
      errors[`guests.${index}`] = `Room ${index + 1} allows up to ${maxGuests} guests; ${totalGuests} are selected.`;
    }
  });
...
function BookingContent() {
  ...
  useEffect(() => {
    ...
    getCartHold(cartHoldToken, locale)
      ...
      .catch((err) => {
        ...
        setStep(1);
        setError('The rooms in your cart are no longer held. Continue to hold them again.');
      });
  ...
  const handleSubmit = async () => {
    ...
    if (invalidCartItem) {
      setError(`The stay dates for ${invalidCartItem.roomTypeName} are no longer valid. Please remove it and select again.`);
      return;
    }
        ...
        {step < 3 && (
          <aside className="sticky top-5 rounded-md border border-stone-200 bg-white p-6 shadow-lg">
            ...
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-slate-800">{`Room cart (${cartItems.length})`}</h3>
            </div>
            ...
            <p className="mt-3 text-center text-xs text-slate-500">{`Rooms are held for ${BOOKING_HOLD_MINUTES} minutes after being added to the cart.`}</p>
          </aside>
        )}
        </div>
      </main>

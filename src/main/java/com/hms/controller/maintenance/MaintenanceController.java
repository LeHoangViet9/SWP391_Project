...
public class MaintenanceController {
        ...
        // THAY ĐỔI: Sử dụng phân quyền dựa trên Authority ('MAINTENANCE_CREATE') thay
        // cho Role ('HOUSEKEEPING'...)
        // giúp đồng bộ hóa phân quyền và hỗ trợ vai trò HOUSEKEEPER/MANAGER tạo phiếu
        // bảo trì thành công
        // b1 API endpoint tạo phiếu bảo trì
        @PostMapping
        @PreAuthorize("hasAuthority('MAINTENANCE_CREATE')")
        public ApiResponse<MaintenanceResponse> createRequest(
                        @Valid @RequestBody MaintenanceRequestCreateDTO dto // @Valid -> tự động kiểm tra định dạng DTO
                                                                            // (ví dụ: không cho rỗng tiêu đề)
        ) {

                Locale locale = LocaleContextHolder.getLocale();

                return ApiResponse.success( // trả về: tạo ApiResponse chứa thông tin thành công và dữ liệu trả về
                                messageSource.getMessage(
                                                "maintenance.create.success", // lấy thông báo
                                                                              // "maintenance.create.success" từ file
                                                                              // message.properties
                                                null,
                                                locale),
                                maintenanceService.createRequest(dto)// gọi tới method createRequest trong
                                                                     // maintenanceService
                );
        }

        // THAY ĐỔI: Đổi defaultValue của sortBy thành CREATED_AT và direction thành
        // DESC để mặc định hiển thị ngày mới nhất trước
        @GetMapping
         ...
         */
        // THAY ĐỔI: Nhận thêm param lý do từ chối (reason) để lưu vết vào chẩn đoán
        // (diagnosis)
        @PostMapping("/{id}/deny")
        @PreAuthorize("hasAuthority('MAINTENANCE_UPDATE')")

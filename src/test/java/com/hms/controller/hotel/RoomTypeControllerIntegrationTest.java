...
//
// import com.hms.common.enums.AccountStatus; // 🛠️ Import enum trạng thái hệ
// thống của bạn
...
//
// // 🛠️ Cập nhật: Thêm trạng thái ACTIVE để phù hợp với hàm findByIdAndStatus
// và các bộ lọc tìm kiếm
...
// mockMvc.perform(get("/api/v1/room-types")
// .header("Accept-Language", "vi") // 🛠️ Dùng Header chuẩn thay vì
// cookie/param để đồng bộ Postman
...
// .locale(Locale.ENGLISH))
// .andExpect(status().isNotFound()); // 🛠️ SỬA LỖI: Exception NotFound phải
// trả về 404
// }
//
// @Test
// voidgetRoomTypeById_NotFound_Vietnamese() throws Exception {
// mockMvc.perform(get("/api/v1/room-types/99999")
// .header("Accept-Language", "vi")
// .locale(Locale.forLanguageTag("vi")))
// .andExpect(status().isNotFound()) // 🛠️ SỬA LỖI: Trả về 404 thay vì 400
// .andExpect(jsonPath("$.success", is(false)))
// .andExpect(jsonPath("$.message", is("Không tìm thấy Loại phòng!"))); // Khớp
// 100% file properties
// }
//
// @Test
...
// .andExpect(jsonPath("$.success", is(false)))
// .andExpect(jsonPath("$.message", is("Dữ liệu nhập vào không hợp lệ! Vui lòng
// kiểm tra lại."))) // Khớp error.validation.failed
// .andExpect(jsonPath("$.data.basePrice", is("Giá cơ bản phải là số dương!")));
// // Khớp roomtype.baseprice.positive
// }
//
// @Test
...
// .locale(Locale.forLanguageTag("vi")))
// .andExpect(status().isConflict()) // 🛠️ SỬA LỖI: Lỗi Conflict trùng lặp phải
// trả về HTTP 409
// .andExpect(jsonPath("$.success", is(false)))
// .andExpect(jsonPath("$.message", is("Tên Loại phòng đã tồn tại!"))); // Khớp
// error.roomtype.exists
// }
//
// @Test
...
// .header("Accept-Language", "vi")
// .locale(Locale.forLanguageTag("vi")))
// .andExpect(status().isConflict()) // 🛠️ SỬA LỖI: Trả về lỗi 409 Conflict
// trùng lặp dữ liệu
// .andExpect(jsonPath("$.success", is(false)))
...
// .andExpect(jsonPath("$.success", is(true)));
//
// // 🛠️ KIỂM TRA LOGIC SOFT DELETE: Sau khi xóa mềm, tìm kiếm bằng ID phải báo
// lỗi 404 lập tức
// mockMvc.perform(get("/api/v1/room-types/" + testRoomType1.getId()))
